define(['app', 'angular'], function (app, angular) {
    ...
    app.factory("AuthenticationLoader", ['Auth', '$location', '$q', '$resource', '$cookieStore', '$http', '$rootScope', 'EngineApi', 'Notifications', '$translate', function (Auth, $location, $q, $resource, $cookieStore, $http, $rootScope, EngineApi, Notifications, $translate) {
        function goToLogin() {
            $location.path('/login');
        };
        ...
        return function () {
            ...
            if (!Auth.username && Token) {
                ...
                EngineApi.DologinValidate().loginToken({
                    token: Token
                }).$promise.then(function (datares) {
                    ...
                    Notifications.addError({
                        'status': 'error',
                        'message': "Token don't get User information"
                    });
                    return;
                });
            } else if (!Auth.username && Tokencode) {
                //   /authorize/loginToken
                EngineApi.DologinValidate().loginCode({
                    code: Tokencode
                }).$promise.then(function (datares) {
                    Auth.saveUser(datares, function (authres) {
                        if (authres) {
